# About File Handler

## Intro

- This is Documentation for File Handler
- We Design File handler to help other components provide callback with data , that can save in a file for future use.
- This principle will help us to pipeline multiple command work together in the future.

### File Handler

#### Features

- file : save media data (video and audio)
- file : save raw-yvu data
- file : save package media fil (such as mp4)
- file : save configure file, that information for component
- file : save resource-information for local or remote resources.
